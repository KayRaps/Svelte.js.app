...
<script>
  ...
  <class="border p-4 rounded-lg shadow hover:shadow-lg bg-white transition-shadow duration-600 text-center" on:click={() => openModal(product)}>
    ...
    <button on:click|stopPropagation={() => toggleFavorite(product.id)}>
      <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="w-6 h-6 text-gray-300" class:text-gray-500={isFavorite(product.id)} viewBox="0 0 24 24">
        <!-- SVG content here -->
      </svg>
    </button>
